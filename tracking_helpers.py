...
def generate_detections(encoder, mot_dir, output_dir, detection_dir=None):
    ...
    for sequence in os.listdir(mot_dir):
        ...
        frame_indices = detections_in[:, 0].astype(np.integer)
        min_frame_idx = frame_indices.astype(np.integer).min()
        max_frame_idx = frame_indices.astype(np.integer).max()
