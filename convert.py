""" Conver checkpoint to models (.pt/.pth/.onnx) """
...
def convert(config: DictConfig):
    ...
    if config.get('convert_to') == 'pytorch':
        ...
        log.info(f"Saved models {config.get('name')}.pt")
    ...
    if config.get('convert_to') == 'onnx':
        ...
        log.info(f"Saved models {config.get('name')}.onnx")
